...
task(
  "deploy-mock-att-module",
  "Deploys MockAttestationModule contract "
).setAction(async (taskArgs, hre) => {
  console.log(`Deploying MockAttestationModule contract to ${network.name}`);
  ...
  const zora = await ethers.getContractFactory("MockAttestationModule");
  const zoraContract = await zora.deploy({ gasPrice: 300000 });
  console.log(
